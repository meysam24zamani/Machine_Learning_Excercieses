...
####################################################################
## 1. A gentle introduction (no code yet!)
####################################################################
...
## in fitting a specific classifier class (e.g. a linear one) to data samples from our problem?
...
## a low-to-medium sized data set, and is very serious for small-sized samples, to the point that it can 
...
plot (data$X1,data$X2,col=data$target, asp = 1)
...
## (in particular, the error of the fitted models)

## there you are ... (we use LDA as learner)
...
## This function returns the AVERAGE CROSS-VALIDATION error
...
# in this case training error and LOOCV coincide; this is typical of very stable classifiers as LDA

legend("topright", legend=c("average k-CV error", "true error", "training error", "LOOCV error"),    
       pch=c(1,1), col=c("black", "blue","red","green"))

# In this case both training error and LOOCV errors coincide; in general the former will be higher
...
## However, you can realize that if, in addition to model selection, you want to give a more
## correct estimation of the true error of a model, then the CV error is not correct. That is why you need a separate TEST sample

## There is no optimal k, but you have to choose one: very often this is a very imprecise function of data sample size and available computational resources (larger values of k entail a heavier computational burden)
