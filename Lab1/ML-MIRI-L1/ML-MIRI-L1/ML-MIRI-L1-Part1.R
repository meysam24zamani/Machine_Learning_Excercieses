...
# the previous code does nothing!
...
# these are then clearly incorrect (BTW, we are using 'with' or direct access to the df)
...
# imputation by nearest neighbours: for every observation with a missing 'Income', we look for the most similar observation (according to the remaining variables) and copy its 'Income' value
...
# As a reminder of dataframe access:
...
# Now, should we treat Age as categorical? probably not ... if we wanted to do so:
